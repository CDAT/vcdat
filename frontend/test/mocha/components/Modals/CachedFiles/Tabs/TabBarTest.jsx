...
describe('TabBarTest.jsx', function() {
    ...
    it('calls switchTab with correct arguments', () => {
        let spy = sinon.spy()
        const tab_bar = shallow(<TabBar switchTab={spy}/>) // use mount because shallow will allow clicks on disabled el. 
        tab_bar.find('#tabbar-file').simulate("click")
        expect(spy.calledWith("file")).to.be.true

        tab_bar.find('#tabbar-esgf').simulate("click")
        expect(spy.calledWith("esgf")).to.be.true 

        tab_bar.find('#tabbar-opendap').simulate("click")
        expect(spy.calledWith("opendap")).to.be.true 

        tab_bar.find('#tabbar-edit').simulate("click")
        expect(spy.calledWith("edit")).to.be.true 
